...
export default async function EmailCoursePage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const courseUrl = `${BASE}/${locale}/free-web-course`;

  return (
    <>
      <CourseJsonLd
        url={courseUrl}
        name="Free Web Course: Digital Presence in 5 Days"
        description="5 emails in 5 days with everything you need to know before investing in your digital presence. Free, no commitment."
      />
      <BreadcrumbJsonLd items={[
        { name: 'Home', item: `${BASE}/${locale}` },
        { name: 'Free Web Course', item: courseUrl },
      ]} />
