...
export default async function AuditoriaWebGratuitaPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const auditUrl = `${BASE}/${locale}/auditoria-web-gratuita`;
  ...
  return (
    <>
      <ServiceJsonLd
        ...
        description="Análisis completo de tu web: velocidad, SEO técnico, conversión y usabilidad. Gratis, sin compromiso."
        price="0"
      />
      <BreadcrumbJsonLd items={[
        { name: 'Inicio', item: `${BASE}/${locale}` },
        { name: 'Auditoría Web Gratuita', item: auditUrl },
      ]} />
