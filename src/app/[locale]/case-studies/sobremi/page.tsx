...
export default function SobreMiCasePage() {
  ...
  const caseUrl = `${BASE}/${locale}/case-studies/sobremi`;

  return (
    <>
      <CaseStudyJsonLd
        url={caseUrl}
        name="Case study: sobremi.online — App with Stripe, n8n and Postgres"
        description="Custom gifting platform built with Stripe payments, n8n automations, Postgres database and production webhooks."
        clientName="sobremi.online"
        reviewBody="A complete platform with real payments, automations and a database, ready to scale."
        reviewerName="Client"
        reviewerTitle="Founder"
      />
      <BreadcrumbJsonLd items={[
        { name: 'Home', item: `${BASE}/${locale}` },
        { name: 'Case Studies', item: `${BASE}/${locale}/case-studies` },
      ...
      <main className="pt-[64px]">
        ...
        <SplitHero
          ...
          leftContent={
            <>
              <Link href="/#case" className="text-[13px] text-[var(--mg)] no-underline font-semibold inline-flex items-center gap-[5px] absolute top-[30px] left-[52px] transition-opacity hover:opacity-75 z-[10]">
                {t('Hero.back')}
              </Link>
            ...
            <div className="relative w-full">
              ...
              <div className="flex flex-col gap-[0]">
                <div className="flex items-center gap-0 mb-[6px]">
                  <div className="bg-[rgba(255,255,255,0.2)] border-[0.5px] border-[rgba(255,255,255,0.35)] backdrop-blur-[14px] rounded-[10px] p-[11px_14px] flex-1">
                    <div className="text-[12px] font-bold text-white mb-[2px]">Cliente completa /crear</div>
                    <div className="text-[10px] text-[rgba(255,255,255,0.55)]">Formulario → POST al backend</div>
                  </div>
                  <div className="px-[8px] text-[rgba(255,255,255,0.4)] font-bold text-[14px] shrink-0">→</div>
                  <div className="bg-[rgba(28,24,40,0.4)] border-[0.5px] border-[rgba(28,24,40,0.2)] backdrop-blur-[14px] rounded-[10px] p-[11px_14px] flex-1">
                    <div className="text-[12px] font-bold text-white mb-[2px]">n8n recibe webhook</div>
                    <div className="text-[10px] text-[rgba(255,255,255,0.55)]">Lógica de decisión por método de pago</div>
                  </div>
                </div>
                
                <div className="mt-[8px] flex flex-col gap-[5px] pl-[16px]">
                  <div className="flex items-center gap-[8px] text-[11px] text-[rgba(255,255,255,0.7)]">
                    <div className="w-[5px] h-[5px] rounded-full shrink-0 bg-[#E8417A]" />
                    Stripe → genera link de pago → webhook confirma → registro en Postgres
                  </div>
                  <div className="flex items-center gap-[8px] text-[11px] text-[rgba(255,255,255,0.7)]">
                    <div className="w-[5px] h-[5px] rounded-full shrink-0 bg-[#F07030]" />
                    Email automático con confirmación detallada al comprador
                  </div>
                  <div className="flex items-center gap-[8px] text-[11px] text-[rgba(255,255,255,0.7)]">
                    <div className="w-[5px] h-[5px] rounded-full shrink-0 bg-[#F5A623]" />
                    Aviso interno a sobremionline@gmail.com con datos del pedido
                  </div>
                  <div className="flex items-center gap-[8px] text-[11px] text-[rgba(255,255,255,0.7)]">
                    <div className="w-[5px] h-[5px] rounded-full shrink-0 bg-[rgba(255,255,255,0.35)]" />
                    Transferencia → email "en revisión" → confirmación manual → email final
                  </div>
                </div>
              </div>
            </div>
          ...
          <PageWrapper>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-[60px] items-start">
              <div>
                ...
                <p className="text-[15px] leading-[1.75] text-[var(--muted)] mb-[16px]" dangerouslySetInnerHTML={{ __html: t('Context.p3') }} />

                <div className="mt-[28px]">
                  <div className="text-[12px] font-bold text-[var(--muted)] tracking-[0.08em] uppercase mb-[12px]">
                    {t('Context.challengesLabel')}
                  </div>
                  <ul className="flex flex-col gap-[10px]">
                    {t.raw('Context.challenges').map((c: string, idx: number) => (
                      <li key={idx} className="flex gap-[12px] items-start text-[14px] text-[var(--dark)] leading-[1.5]">
                        <div className="w-[24px] h-[24px] rounded-[6px] bg-[rgba(232,65,122,0.1)] flex items-center justify-center text-[10px] font-extrabold text-[var(--mg)] shrink-0 mt-[1px]">
                          {idx + 1}
                        </div>
