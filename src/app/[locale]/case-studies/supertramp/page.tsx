...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return {
    ...
    alternates: {
      canonical: `https://vacaredigitalsolutions.com/${locale}/case-studies/supertramp`,
    }
  };
}
...
export default function SupertrampCasePage() {
  ...
  return (
    <>
      <CaseStudyJsonLd
        url={caseUrl}
        name="Case study: Supertramp Hostels — 30% of direct bookings from the website"
        description="Web redesign + strategic SEO for Supertramp Hostels in Cusco and Machu Picchu. 30% of total bookings came directly from their own site within 6 months."
        clientName="Supertramp Hostels"
        reviewBody="30% of our revenue now comes from our own website. The difference is enormous."
        reviewerName="Theo"
        reviewerTitle="General Manager"
      />
              ...
              <div className="flex flex-col gap-[10px]">
                {t.raw('Hero.results').map((rc: any, idx: number) => (
                  <div key={idx} className="bg-[rgba(255,255,255,0.12)] border-[0.5px] border-[rgba(255,255,255,0.22)] backdrop-blur-[14px] rounded-[14px] p-[16px_18px] flex items-center gap-[14px]">
                    <div className="text-[32px] font-extrabold text-white tracking-[-1.5px] leading-none shrink-0 min-w-[60px]">{rc.num}</div>
                    <div>
          ...
          <PageWrapper className="relative z-10">
            ...
            <div className="flex flex-col gap-0">
              {t.raw('Strategy.steps').map((step: any, idx: number, arr: any[]) => (
                <div key={idx} className="grid grid-cols-[52px_1fr] md:grid-cols-[64px_1fr] gap-0 relative">
                  <div className="flex flex-col items-center pt-[4px]">
                    <div className="w-[48px] h-[48px] rounded-[12px] grad-bg flex items-center justify-center text-[12px] font-extrabold text-white shrink-0 z-10 shadow-[0_6px_20px_rgba(232,65,122,0.2)]">
                      {step.num}
                    </div>
                    {idx < arr.length - 1 && (
                      <div className="flex-1 w-[2px] bg-gradient-to-b from-[rgba(232,65,122,0.2)] to-[rgba(245,166,35,0.05)] mt-[8px]" />
                    )}
                  </div>
                  <div className="bg-[rgba(247,246,242,0.04)] border-[0.5px] border-[rgba(247,246,242,0.07)] rounded-[16px] p-[24px_28px] ml-[16px] mb-[12px] transition-colors hover:border-[rgba(232,65,122,0.3)] relative overflow-hidden group">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-[14px] mb-[14px]">
              {t.raw('Result.big').map((rb: any, idx: number) => (
                <div key={idx} className={`${rb.featured ? 'grad-bg border-none md:row-span-2' : 'bg-[rgba(247,246,242,0.04)] border-[0.5px] border-[rgba(247,246,242,0.08)] transition-colors hover:border-[rgba(232,65,122,0.3)]'} rounded-[20px] p-[40px]`}>
                  ...
                  <div className={`text-[15px] font-bold mb-[6px] ${rb.featured ? 'text-white' : 'text-[var(--dark)]'}`}>{rb.label}</div>
                  <div className={`text-[13px] leading-[1.6] ${rb.featured ? 'text-[rgba(255,255,255,0.65)]' : 'text-[var(--muted)]'}`}>{rb.sub}</div>
                </div>
              ))}
            </div>
